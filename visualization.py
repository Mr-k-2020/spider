...
data = pd.read_csv('/data2.csv',encoding='gb18030')
...
for title in data.title:
    str = str + title
wordcloud = WordCloud(font_path='C:\\Windows\\Fonts\\simsun.ttc',background_color="white",width=1500, height=900).generate(str)#font_path自行下载的字体后的路径，否则会乱码
plt.imshow(wordcloud, interpolation='bilinear')
plt.axis("off")
ax[1].set_title('记录里所有视频标题内容的词云图',fontsize=20,pad=20)

plt.subplots_adjust(hspace =0.2)
plt.savefig("E:\\python_code\\pycharm_code\\B_history_cloud\\fig.png", dpi=300)
plt.show()